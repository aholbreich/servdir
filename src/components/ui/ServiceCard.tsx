...
export function ServiceCard({ service }: Props) {
  ...
  return (
    <Card className="flex h-full flex-col px-4 pt-4 pb-3 sm:px-5 sm:pt-5">
      <div>
        <h2 className="text-title min-w-0">
          <a href={toAppPath(`/services/${service.slug}`)}>{service.data.name}</a>
        </h2>
        <span className="text-meta inline-flex items-center gap-1.5 text-muted-foreground">
        ...
        {service.data.tags && service.data.tags.length > 0 && (
          <p className="text-meta mt-1.5 flex flex-wrap gap-x-3 gap-y-1">
      ...
      <p className="text-body mt-3 line-clamp-2 text-muted-foreground">
        {summary}
      </p>

      <div className="mt-auto flex flex-wrap items-center justify-between gap-x-4 gap-y-2 border-t border-border/70 pt-3 text-sm">
